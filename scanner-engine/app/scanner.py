...
JS_PATTERNS = {
    ...
    "RSA": {
        ...
        "severity": "CRITICAL",
        "description": "RSA broken by Shor's algorithm on quantum computers — all key sizes vulnerable",
    ...
    "ECC": {
        ...
        "severity": "CRITICAL",
        "description": "ECC broken by Shor's algorithm — discrete log problem solvable on quantum computers",
    ...
    "DSA": {
        "patterns": [
            r"\bDSA\b",
            r"\bdsa\b",
            r"generateKeyPair(?:Sync)?\s*\(\s*['\"]dsa['\"]",
        ],
        "severity": "CRITICAL",
        "description": "DSA broken by Shor's algorithm — discrete logarithm problem solvable",
    ...
    "DH": {
        ...
        "severity": "CRITICAL",
        "description": "Diffie-Hellman broken by Shor's algorithm — discrete log problem solvable",
        "replacement": "CRYSTALS-Kyber KEM",
        "nist_ref": "NIST FIPS 203",
    },
    "DES": {
        "patterns": [
            r"createCipher(?:iv)?\s*\(\s*['\"]des",
            r"\bDES\b",
            r"\bdes\b",
            r"\b3DES\b",
            r"\bTripleDES\b",
    ...
    "HARDCODED_KEY": {
        "patterns": [
            r"""(?:password|secret|api_key|apikey|token|SECRET)\s*=\s*['"][^'"]{8,}['"]""",
            r"""private_key\s*=\s*['"]""",
        ],
        "severity": "HIGH",
    ...
    "XSS": {
        ...
        "severity": "HIGH",
        "description": "DOM-based XSS — unsanitized content injection",
        "replacement": "Use textContent or a DOMPurify sanitizer",
...
PY_PATTERNS = {
    "MD5": {
        "patterns": [r"hashlib\.md5", r"\bmd5\(", r"MD5\.new", r"hashlib\.md5"],
        "severity": "CRITICAL",
        "description": "MD5 weakened by Grover's algorithm — effective security drops from 128 to 64 bits",
        "replacement": "hashlib.sha3_256()",
        "nist_ref": "NIST SP 800-131A Rev 2",
    },
    "SHA1": {
        "patterns": [r"hashlib\.sha1", r"\bsha1\(", r"SHA\.new", r"SHA1"],
        "severity": "CRITICAL",
        "description": "SHA1 broken for collision resistance — demonstrated by Google's SHAttered attack",
        "replacement": "hashlib.sha3_256()",
        "nist_ref": "NIST SP 800-131A Rev 2",
    },
    "RSA": {
        ...
        "severity": "CRITICAL",
        "description": "RSA broken by Shor's algorithm — all key sizes (1024, 2048, 4096) are vulnerable",
    ...
    "ECC": {
        ...
        "severity": "CRITICAL",
        "description": "ECC broken by Shor's algorithm — discrete logarithm problem solvable",
    ...
    "DSA": {
        "patterns": [
            ...
            r"from\s+.*import\s+dsa",
            r"\bDSA\b",
        ],
        "severity": "CRITICAL",
        "description": "DSA broken by Shor's algorithm — discrete logarithm problem solvable",
        "replacement": "CRYSTALS-Dilithium or FALCON via liboqs-python",
        "nist_ref": "NIST FIPS 204",
    },
    "DES": {
        "patterns": [
    ...
    "HMAC_WEAK": {
        ...
        "severity": "CRITICAL",
        "description": "HMAC with weak hash (MD5/SHA1) — quantum-vulnerable authentication",
        "replacement": "hmac.new() with hashlib.sha3_256",
        "nist_ref": "NIST SP 800-131A Rev 2",
    },
    "WEAK_AES": {
    ...
    "HARDCODED_KEY": {
        "patterns": [
            r"""(?:password|secret|api_key|apikey|token|SECRET_KEY|PRIVATE_KEY)\s*=\s*['"][^'"]{8,}['"]""",
            r"""private_key\s*=\s*['"]""",
        ],
        "severity": "HIGH",
